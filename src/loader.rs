...
impl ImageLoader {
  ...
  pub fn load(&self) -> bool {
    ...
    if let Some(extract_command) = self.command_list.get(file_ext.as_str()) {
      let cmd = format!("docker run --rm --volume {}:/mybackup -v {}:/backup alpine sh \
      ...
      let status = Command::new(splitted[0]).args(&splitted[1..])
                                            .status()
  ...
  fn get_extension_from_filename(&self) -> (String, String) {
    ...
    let path = Path::new(&self.image_path).canonicalize()
